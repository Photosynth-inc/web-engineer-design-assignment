...
このリポジトリには、課題の題材となるサービスの現状の仕様（スキーマとAPI）と、提出物であるADR（Architecture Decision Record）のテンプレートが入っています。
...
題材は、企業向けサービスの管理APIを想定した架空のシステムです。
実在のサービスの構造を写したものではありませんが、制約は運用中のシステムでよく起きる形にしてあります。
次のデータを扱っています。
...
- 利用者は本番で数十万件あり、サービスを止めてのデータ移行はできません。そのため、移行のあいだは旧項目（`organization_id`）と新しい構造が同時に存在する期間ができます。その期間に外部の連携先と管理画面が正しく動き続けるための扱いを、移行の手順に含めてください
...
移行の手順は、段階の粒度で書けば足ります。
SQLやツールの詳細は要りません。
無停止での移行の一般的な進め方を知らなければ、調べて構いません。
参考にした資料はADRの備考に書いてください。
